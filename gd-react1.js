...
const runMethod = async () => {
  ...
  await page.goto(`https://www.glassdoor.com/Job/react-jobs-SRCH_KE0,5.htm?radius=25&fromAge=7&minSalary=64000&gocId=100224`)
  ...
  if (passTest) {
    ...
    while (startPaginationNumber <= endPaginationNumber) {
      await page.goto(`https://www.glassdoor.com/Job/react-jobs-SRCH_KE0,5_IP${startPaginationNumber}.htm?radius=25&fromAge=7&minSalary=64000&gocId=100224`)
      // await page.waitFor(2000);
    ...
    fs.writeFile(`./jobs/gd-react1.js`, JSON.stringify(allJobs), function (err) {
      if (err) {
        console.log(err);
      }
      else {
        console.log("Output saved to /gd-react1.js.");
      }
    });
  }
